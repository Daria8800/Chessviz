...
void moveFigure(char turn[], char pole[][9])
{
    if (pole[x_1 - 1][y_1] == turn[0]) {
        ...
        cout << "Entry incorrect, you tried to go not by figures from the back "
                "row"
             << endl
             << endl;
}
...
bool Check(char turn[], char pole[][9], int Qualifier)
{
    ...
    if (x_1 <= BoundMin || x_1 >= BoundMax || y_1 <= BoundMin || y_1 >= BoundMax
        || x_2 <= BoundMin || x_2 >= BoundMax || y_2 <= BoundMin
        || y_2 >= BoundMax || (pole[x_1 - 1][y_1] == ' ')) {
        cout << "Entry incorrect" << endl << endl;
        return false;
    }
    return true;
